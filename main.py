...
# Set fonts
font.init()
font_counter = font.SysFont('Arial', 44) # This is for score and missed
font_win = font.SysFont('Arial', 80) # This is for win message
win_text = font_win.render("YOU WIN!", True, GREEN)
font_lose = font.SysFont('Arial', 80) # This is for lose message
...
while game:
    ...
    for e in event.get():
        if e.type == QUIT:
            ...
            if e.key == K_SPACE:
                if num_fire <= max_fire and reload_time == False:
                    num_fire += 1
                    fire_sound.play()
                    rocket.fire()
                if num_fire > max_fire and reload_time == False:
    ...
    if not finish:
        ...
        text_missed = font_counter.render("Missed: " + str(missed), True, RED)
        ...
        if reload_time == True:
            ...
            if now_time - last_time <= 5:
                reload_text = font_counter.render("Reloading...", True, BLACK)
                window.blit(reload_text, (200,200))
            else:
                num_fire = 0
                reload_time = False
